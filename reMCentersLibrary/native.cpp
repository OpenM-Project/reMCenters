...
namespace reMCentersNative {
	namespace DllMethod {
		...
		bool Patchx64Dll() {
			bool result = false;
			if (!reMCentersNative::Utility::IsLittleEndian()) return false;  // this line ensures code only runs when running in little endian mode, we dont support big endian for now
			...
			auto recordPex64= reMCentersNative::Utility::GetRecordPEx64(file);
			...
			if (!file.read(rDataBuffer.data(), rDataBuffer.size()))  goto exit;
				...
				auto storeApplicenseGetTrialKeywordPosition = reMCentersNative::Utility::find_pattern_in_array(rDataBuffer.data(), Windows_Services_Store_StoreApplicense_getTrial_keyword, rDataBuffer.size(), 55);
				if (storeApplicenseGetTrialKeywordPosition != -1) {
					...
					while (true) {
						storeApplicenseGetTrialKeywordReferenceOffset = reMCentersNative::Utility::find_pattern_in_array(textBuffer.data(), x64LeaReferenceOpcodePrefix, textBuffer.size(), 3, storeApplicenseGetTrialKeywordReferenceOffset + 1);
						...
						if (textBaseAddress + storeApplicenseGetTrialKeywordReferenceOffset + offsetInLea + SizeOfx64LeaOpcode == storeApplicenseGetTrialKeywordPosition) {
							...
							while (ZYAN_SUCCESS(ZydisDisassembleIntel(
								...
								if (reMCentersNative::Utility::stringStartsWith(opcode, "mov cl, [rcx")) {
									if (!reMCentersNative::Utility::writeBytesAtAddress(file, runtime_address, CrackOpcode, 3)) goto exit;
									break;
								}
								if (reMCentersNative::Utility::areStringsEqual(opcode, "mov cl, 0x00"))
									break;

								if (reMCentersNative::Utility::stringStartsWith(opcode, "ret"))
									goto exit;
				...
				auto licenseInformationServerGetTrialKeywordPosition = reMCentersNative::Utility::find_pattern_in_array(rDataBuffer.data(), Windows_ApplicationModel_Store_LicenseInformationServer_getTrial_keyword, rDataBuffer.size(), 72);
				...
				char* leaOffsetter = reinterpret_cast<char*>(&offsetInLea);
				while (true) {
					licenseInformationServerGetTrialKeywordReferenceOffset = reMCentersNative::Utility::find_pattern_in_array(textBuffer.data(), x64LeaReferenceOpcodePrefix, textBuffer.size(), 3, licenseInformationServerGetTrialKeywordReferenceOffset + 1);
					...
					if (textBaseAddress+ licenseInformationServerGetTrialKeywordReferenceOffset + offsetInLea + SizeOfx64LeaOpcode == licenseInformationServerGetTrialKeywordPosition) {
						...
						while (ZYAN_SUCCESS(ZydisDisassembleIntel(
							...
							if (reMCentersNative::Utility::stringStartsWith(opcode, "setnz cl")) {
		...
		bool Patchx86Dll() {
			bool result = false;
			if (!reMCentersNative::Utility::IsLittleEndian()) return false;
			...
			auto recordPEx86 = reMCentersNative::Utility::GetRecordPEx86(file);
			...
			if (!file.read(textSectionBuffer.data(), textSectionBuffer.size())) goto exit;
				...
				size_t storeApplicenseGetTrialKeywordPosition = reMCentersNative::Utility::find_pattern_in_array(textSectionBuffer.data(), Windows_Services_Store_StoreApplicense_getTrial_keyword, textSectionBuffer.size(), 55) + baseAddress;
				...
				if (storeApplicenseGetTrialKeywordPosition != std::string::npos) {
					...
					*x86PushOffset = storeApplicenseGetTrialKeywordPosition;
					...
					while (!wroteSome) {
						storeApplicenseGetTrialKeywordReferenceOffset = reMCentersNative::Utility::SearchBytePattern(textSectionBuffer, x86PushOpcode, 5, storeApplicenseGetTrialKeywordReferenceOffset + 1);
						...
						while (ZYAN_SUCCESS(ZydisDisassembleIntel(
							...
							auto opcode = instruction.text;
							if (reMCentersNative::Utility::stringStartsWith(opcode, "mov cl, [ecx")) {
								if (!reMCentersNative::Utility::writeBytesAtAddress(file, runtime_address, CrackOpcode, 3)) goto exit;
								wroteSome = true;
								break;
							}
				...
				// windows application model store part
				auto licenseInformationServerGetTrialKeywordPosition = reMCentersNative::Utility::find_pattern_in_array(textSectionBuffer.data(), Windows_ApplicationModel_Store_LicenseInformationServer_getTrial_keyword, textSectionBuffer.size(), 72) + baseAddress;
				...
				*x86EbxOffset = licenseInformationServerGetTrialKeywordPosition;
				...
				bool wroteSome = false;
				while (!wroteSome) {
					licenseInformationServerGetTrialKeywordReferenceOffset = reMCentersNative::Utility::SearchBytePattern(textSectionBuffer, x86EbxOpcode, 5, licenseInformationServerGetTrialKeywordReferenceOffset + 1);
